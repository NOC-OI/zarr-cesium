...
export function deriveRectangleAndScheme(crs: string, xyLimits: any, levelMetadata: any, zarrArray: any, dimIndices: any): {
  ...
  try {
    ...
    const dx = (xMax - xMin) / (levelMetadata.get(0)?.width ?? zarrArray.shape[dimIndices.lon.index] - 1);
    const dy = (yMax - yMin) / (levelMetadata.get(0)?.height ?? zarrArray.shape[dimIndices.lat.index] - 1);
