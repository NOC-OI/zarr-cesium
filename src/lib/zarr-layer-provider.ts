...
export class ZarLayerProvider implements Cesium.ImageryProvider {
  ...
  private rebuildColorRamp(): void {
    if (!this.gl) return;
    this.colorTexture = createColorRampTexture(this.gl, this.colorScale.colors);
  }
  ...
  async requestImage(
    ...
    try {
      ...
      const data = (await ZarLayerProvider.throttle(() =>
  ...
  private async renderWithWebGL(
    ...
    if (await ZarLayerProvider.checkImageBitmapSupport()) {
      try {
        ...
        ZarLayerProvider.supportsImageBitmap = false;
      }
    }
