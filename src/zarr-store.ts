...
export class ZarrStore {
  ...
  variable: string;
  dimensionNames: DimensionNamesProps;
  private explicitBounds: BoundsProps | null;
  coordinateKeys: string[];
  ...
  private _levelInfoCache = new Map<string, Promise<{ width: number; height: number }>>();
  ...
  private _arrayHandles = new Map<string, Promise<zarr.Array<zarr.DataType, Readable>>>();
  ...
  /**
   * Map a Cesium zoom level (0..maxCesium) to a multiscale level path.
   * Returns undefined if there is no multiscale pyramid (single-scale).
   */
  resolveLevelForCesium(cesiumLevel: number, maxCesiumLevel: number): string | undefined {
    if (!this.levels || this.levels.length === 0) return undefined;

    // If you ever want different policies for tiled vs untiled, branch here.
    const normalized = maxCesiumLevel > 0 ? cesiumLevel / maxCesiumLevel : 0;
    const idx = Math.floor(normalized * (this.levels.length - 1));

    return this.levels[Math.max(0, Math.min(this.levels.length - 1, idx))];
  }
    ...
    await Promise.all(
      this.coordinateKeys.map(async key => {
        try {
          ...
          this.coordinates[key] = Array.from(chunk.data as ArrayLike<number | string>);
        } catch (err) {
  ...
  async getArrayForCesiumLevel(
    cesiumLevel: number,
    maxCesiumLevel: number
  ): Promise<{
    ...
    const levelPath = this.resolveLevelForCesium(cesiumLevel, maxCesiumLevel) ?? null;

    // Open array (your store already caches handles in _arrayHandles)
    const array = levelPath ? await this.getLevelArray(levelPath) : await this.getArray();

    // Width/height using store dimIndices (already computed)
    // Use levelPath’s metadata when needed; otherwise just read shape.
    const lonIdx = this.dimIndices.lon?.index;
    const latIdx = this.dimIndices.lat?.index;

    if (lonIdx === undefined || latIdx === undefined) {
      // Fallback: assume last 2 dims are y,x (not ideal, but safe-ish)
      const height = array.shape[array.shape.length - 2];
      const width = array.shape[array.shape.length - 1];
      return { levelPath, array, width, height };
    }

    const width = array.shape[lonIdx];
    const height = array.shape[latIdx];
  ...
  async getLevelArray(level: string): Promise<zarr.Array<zarr.DataType, Readable>> {
    const key = `${level}/${this.variable}`;
  ...
  async getUntiledLevelMetadata(levelAsset: string): Promise<{
    ...
    try {
      if (this.version === 3) {
        ...
        const zattrs = (await this._getJSON(`/${arrayKey}/.zattrs`).catch(() => ({}))) as {
          scale_factor?: number;
          add_offset?: number;
        };
      ...
      console.warn(`[ZarrStore] Failed to load per-level metadata for ${arrayKey}:`, err);
    }
  ...
  private async _getArray(key: string): Promise<zarr.Array<zarr.DataType, Readable>> {
    if (!this.root) {
  ...
  private isConsolidatedStore(store: ZarrStoreType | null): store is ConsolidatedStore {
    return store !== null && typeof (store as ConsolidatedStore).contents === 'function';
  }

  private async _loadV2() {
    const cacheKey = `v2:${this.source}`;
    let zmetadata = ZarrStore._cache.get(cacheKey) as ZarrV2ConsolidatedMetadata | undefined;
    if (!zmetadata) {
      if (this.isConsolidatedStore(this.store)) {
        ...
        try {
          zmetadata = (await this._getJSON('/.zmetadata')) as ZarrV2ConsolidatedMetadata;
          ZarrStore._cache.set(cacheKey, zmetadata);
        } catch {
    ...
    const rootAttrs = zmetadata.metadata['.zattrs'] as ZarrV2Attributes | undefined;
    ...
    let zattrs = v2Metadata.metadata[`${basePath}/.zattrs`] as ZarrV2Attributes | undefined;
    let zarray = v2Metadata.metadata[`${basePath}/.zarray`] as ZarrV2ArrayMetadata | undefined;

    if (!zattrs || !zarray) {
      [zattrs, zarray] = await Promise.all([
        zattrs
          ? Promise.resolve(zattrs)
          : (this._getJSON(`/${basePath}/.zattrs`).catch(() => ({})) as Promise<ZarrV2Attributes>),
        zarray
          ? Promise.resolve(zarray)
          : (this._getJSON(`/${basePath}/.zarray`) as Promise<ZarrV2ArrayMetadata>)
      ]);
  ...
  private async _loadV3() {
    ...
    let metadata = ZarrStore._cache.get(metadataCacheKey) as ZarrV3GroupMetadata | undefined;
    if (!metadata) {
      ...
      if (metadata.consolidated_metadata?.metadata) {
        for (const [key, arrayMeta] of Object.entries(metadata.consolidated_metadata.metadata)) {
    ...
    let arrayMetadata = ZarrStore._cache.get(arrayCacheKey) as ZarrV3ArrayMetadata | undefined;
    if (!arrayMetadata) {
      arrayMetadata = (await this._getJSON(`/${arrayKey}/zarr.json`)) as ZarrV3ArrayMetadata;
      ZarrStore._cache.set(arrayCacheKey, arrayMetadata);
    }
    this.arrayMetadata = arrayMetadata;

    const attrs = arrayMetadata.attributes as Record<string, unknown> | undefined;
    ...
    const shardedChunkShape =
      isSharded && arrayMetadata.codecs?.[0]?.configuration
        ? (arrayMetadata.codecs[0].configuration as { chunk_shape?: number[] }).chunk_shape
        : undefined;
    ...
    const legacyChunks = Array.isArray(arrayMetadata.chunks) ? arrayMetadata.chunks : undefined;
  ...
  private async _computeDimIndices() {
    ...
    this.dimIndices = identifyDimensionIndices(this.dimensions, this.dimensionNames);
  ...
  private async _findBoundsLevel(): Promise<string | undefined> {
    ...
    try {
      const firstArray = await openArray(this.root.resolve(`${firstLevel}/${this.variable}`));
      const lastArray = await openArray(this.root.resolve(`${lastLevel}/${this.variable}`));
  ...
  private async _loadSpatialMetadata() {
    ...
    if (this.multiscaleType === 'untiled' && !this._crsFromMetadata && this.xyLimits) {
      const maxAbsX = Math.max(Math.abs(this.xyLimits.xMin), Math.abs(this.xyLimits.xMax));
      if (maxAbsX > 360) {
        this.crs = 'EPSG:3857';
      }
    }
  ...
  private _getPyramidMetadata(
    ...
    if (Array.isArray(multiscales) && multiscales[0]?.datasets?.length) {
      ...
      const crs: CRS = (datasets[0].crs as CRS) === 'EPSG:4326' ? 'EPSG:4326' : 'EPSG:3857';
      ...
      if (tileSize) {
        ...
        const consolidatedMeta = (this.metadata as ZarrV3GroupMetadata)?.consolidated_metadata
          ?.metadata;
        ...
        this.untiledLevels = levels.map(level => {
          ...
          if (consolidatedMeta) {
            ...
            const arrayMeta = consolidatedMeta[arrayKey] as ZarrV3ArrayMetadata | undefined;
            if (arrayMeta?.shape) {
              ...
              const shardChunks = arrayMeta.codecs?.find(c => c.name === 'sharding_indexed')
   ...
   * @see https://github.com/zarr-conventions/multiscales
   */
  private _parseUntiledMultiscale(metadata: UntiledMultiscaleMetadata): PyramidMetadata {
